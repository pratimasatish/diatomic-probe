...
if args.fmt == "xyz":
    print "{}".format(n_molecules * n_chain + n_molecules * 16 + 2 * n_hexatoms + n_bottom)
    print "Comment Line"
    nmol = 0
    # atomic coordinates
    for i in range(nx):
    ...
    for i in range(n_hexatoms):    
        if ((i%6 == 0) or ((i+1)%6 == 0)):
            print "5	{:4.5f}	{:4.5f}	{:4.5f}".format( coords[i,0], coords[i,1], coords[i,2] )
        else:
            print "6	{:4.5f}	{:4.5f}	{:4.5f}".format( coords[i,0], coords[i,1], coords[i,2] )
...
if args.fmt == "lmp":
    ...
    print "{}	atoms".format( n_molecules * n_chain + n_molecules * 16 + 2 * n_hexatoms + n_bottom )
    print "{}	bonds".format( n_molecules * n_bonds + n_molecules + 2 * n_hexmolecules * n_hexbonds + n_bottommolecules * n_bonds )
    print "{}	angles".format( n_molecules * n_angles + 2 * n_hexmolecules * n_hexangles + n_bottommolecules * n_angles )
    print "{}	dihedrals".format( n_molecules * n_dihedrals + 2 * n_hexmolecules * n_hexdihedrals + n_bottommolecules * n_dihedrals )
